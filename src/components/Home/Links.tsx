    ...
    return (
        <section className='flex flex-row justify-between w-full'>
            {links.map((link: ILink, index: number) => {
                if(index <= 0) return;
                return (
                    <NavLink to={link.address} className='border-l-[.3px] border-dark-hacki w-full p-8 duration-300 group hover:bg-bg-browney flex flex-col gap-3'>
                        <p className='text-green font-jetbrains'>0{index}</p>
